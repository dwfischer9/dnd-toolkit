...
export const getFirstActiveCreatureId = (creatures: Creature[]) =>
  sortCreatures(creatures)[0]?.id ?? '';

export const rollCreatureInitiative = (creatures: Creature[], roller: () => number = rollD20) =>
...
export const getCreatureSaveBonus = (creature: Creature, abilityKey: AbilityKey) => {
  const proficientBonus = creature.savingThrowBonuses?.[abilityKey];
  if (proficientBonus !== undefined) {
    return { bonus: proficientBonus, isProficient: true };
  }

  const abilityScore = creature.abilityScores?.[abilityKey];
  if (abilityScore === undefined) {
    return { bonus: 0, isProficient: false };
  }

  return { bonus: getAbilityModifier(abilityScore), isProficient: false };
};
...
export const addCreatureToEncounter = (
  ...
  const resolvedId = creature.id && !occupiedIds.has(creature.id) ? creature.id : idFactory();

  const nextCreature = {
    ...creature,
    id: resolvedId,
    initiative: creature.initiative > 0 ? creature.initiative : creature.isPlayer ? 0 : roller(),
...
export const expireCreatureEffectsAtRoundBoundary = (
  creature: Creature,
  round: number,
): Creature => ({
  ...creature,
  effects: (creature.effects ?? []).filter((effect) => {
    if (effect.source !== CreatureEffectSources.Encounter) {
      return true;
    }
    if (effect.timing === 'turn_start') {
      return true;
    }
    if (effect.durationRounds === undefined || effect.anchorRound === undefined) {
      return true;
    }
    return round - effect.anchorRound < effect.durationRounds;
  }),
});

export const applyTurnStartEffectUpdates = (
  creatures: Creature[],
  activeCreatureId: string,
): Creature[] =>
  creatures.map((creature) => {
    const nextEffects = (creature.effects ?? [])
      .map((effect) => {
        if (
          effect.source !== CreatureEffectSources.Encounter ||
          effect.timing !== 'turn_start' ||
          !effect.anchorCreatureId ||
          effect.durationRounds === undefined ||
          effect.anchorCreatureId !== activeCreatureId
        ) {
          return effect;
        }
        ...
        return {
          ...effect,
          elapsedTurnStarts: (effect.elapsedTurnStarts ?? 0) + 1,
        };
      })
      .filter((effect) => {
        if (
        ...
        return (effect.elapsedTurnStarts ?? 0) < effect.durationRounds;
      });
...
export const pruneEffectsByRemovedAnchor = (creatures: Creature[], removedCreatureId: string) => {
  let removedCount = 0;
  const nextCreatures = creatures.map((creature) => ({
    ...creature,
    effects: (creature.effects ?? []).filter((effect) => {
      const shouldRemove = effect.anchorCreatureId === removedCreatureId;
      if (shouldRemove) {
        removedCount += 1;
      }
      return !shouldRemove;
  ...
  return { creatures: nextCreatures, removedCount };
};

export const getDefenseConflicts = (effects: CreatureEffect[]) => {
  const categoriesByScope = new Map<string, Set<CreatureDefenseEffectCategory>>();
  for (const effect of effects) {
    if (effect.category === 'condition') {
      continue;
    }
    const categories =
      categoriesByScope.get(effect.scope) ?? new Set<CreatureDefenseEffectCategory>();
    categories.add(effect.category);
    categoriesByScope.set(effect.scope, categories);
  }

  return [...categoriesByScope.entries()]
    .filter(([, categories]) => categories.size > 1)
    .map(([scope, categories]) => ({ scope, categories: [...categories] }));
};
