...
export default function CreatureCard({
  ...
  return (
    <div
      data-creature-card="true"
      className={`relative rounded-2xl border p-3 shadow-md transition-all ${
        creature.isActive
          ? 'border-amber-300/55 bg-stone-900/95 shadow-amber-900/20'
          : 'border-amber-100/10 bg-stone-950/70'
      }`}
    >
      {creature.isActive && (
        <div className="pointer-events-none absolute inset-y-3 left-0 w-1 rounded-r-full bg-amber-300/80" />
      )}
      <div className="flex items-start justify-between gap-3">
        <div className="min-w-0 space-y-2">
          <div className="flex flex-wrap items-center gap-2">
            <h3 className="text-base font-semibold text-white">{creature.name}</h3>
            <span className={`rounded-full border px-2 py-0.5 text-xs ${badgeClassName}`}>
              {creature.isPlayer ? 'PC' : 'Creature'}
            </span>
            <span className="rounded-full border border-white/10 bg-black/15 px-2 py-0.5 text-xs text-gray-300">
              Source: {sourceLabel}
            </span>
          </div>
        ...
        <div className="flex shrink-0 items-center gap-1.5">
          {onEditCreature && (
            <button
              ...
              className="rounded-full border border-white/10 bg-black/25 px-2.5 py-1 text-xs text-white transition hover:bg-white/10"
            >
              Edit
            </button>
          )}
          <button
            ...
            className="rounded-full border border-amber-400/25 bg-amber-400/10 px-2.5 py-1 text-[11px] font-semibold text-amber-100 transition hover:bg-amber-400/20"
          >
            + Effect
          </button>
          {featureCount > 0 && (
          ...
          <button
            ...
            className="rounded-full border border-red-400/20 bg-red-500/20 px-2.5 py-1 text-xs text-white transition hover:bg-red-500/30"
          >
            x
          </button>
        </div>
      ...
      <div className="mb-2 mt-2.5 flex items-center gap-1.5">
        <div className="relative h-7 flex-1 overflow-hidden rounded-full border border-white/10 bg-black/40">
        ...
        <div className="inline-flex flex-wrap items-center gap-1 rounded-2xl border border-white/10 bg-black/35 p-1">
          <button
      ...
      {(abilityRows.length > 0 || skillEntries.length > 0) && (
        <div className="mb-2.5 space-y-2 rounded-xl border border-white/10 bg-black/20 p-2.5">
