...
const parseDescriptionDamage = (description: string) => {
  const firstHitMatch = description.match(
    /Hit:\s*\d+\s*\(([^)]+)\)\s*([a-zA-Z ]+?)\s*damage/i
  )
...
const mapSavingThrow = (dc?: { dc_type: { name: string }; dc_value: number; success_type: string }): CreatureFeatureSavingThrow | undefined =>
  dc
    ? {
...
const mapFeature = (
  ...
  const { damage: descDamage, versatileDamage: descVersatile } = parseDescriptionDamage(feature.desc)
...
export const buildCreatureFeatureGroups = (apiCreature: ApiCreature): CreatureFeatureGroup[] =>
  [
    buildGroup(apiCreature.index, CreatureFeatureTypes.Trait, 'Traits', apiCreature.special_abilities),
    buildGroup(apiCreature.index, CreatureFeatureTypes.Action, 'Actions', apiCreature.actions),
    buildGroup(apiCreature.index, CreatureFeatureTypes.Legendary, 'Legendary Actions', apiCreature.legendary_actions),
